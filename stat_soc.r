...
loessModelAgg <- loess(aggdata$Out ~ aggdata$Sens)
...
aggdata <- as.data.frame(aggdata) 
head(aggdata)
aggdata$Raggio <- seq(from=1, to=10, 1)
...
my.count <- seq(from=1, to=10, by=1)
pred <- predict(loessModelAgg,my.count,se=TRUE)
...
#aggregated data
plot(aggdata$Out ~ aggdata$Raggio,type="p",lwd=3,ylab="Produzione Energetica ( kW )", xlab="Raggio",xlim=c(0,10),cex.lab=0.9) 
grid(lwd=2)
